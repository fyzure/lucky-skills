...
For NAT-PMP mapping behavior, use `tools/lucky_natpmp_ci_probe.py` rather than manufacturing NAT on the production RS host. It is GitHub-Actions-only and places pinned Lucky 3.0.0 on a Docker `--internal` LAN while a separate veth-backed network namespace acts as a synthetic WAN. Owned stdlib fixtures provide the STUN Binding server, NAT-PMP UDP/5351 gateway and UDP echo target. Lucky configuration still changes only through HTTP APIs: the disposable udp4 rule uses `NatPMP=true`, `UPnP=false` and `AutoOptionsFirewall=false`. Require a real NAT-PMP UDP add whose internal port equals Lucky `ListenPort`, then require a random marker from the WAN namespace to traverse the assigned external-port mapping relay -> Lucky -> echo target -> Lucky -> relay and return byte-for-byte. Repeated add/renew for the same internal mapping must retain the assigned external port. Disabling/deleting the rule must produce a lifetime=0 NAT-PMP delete and remove the relay before the STUN rule/module baselines are restored. The probe uses no Internet route, public STUN service, real router, iptables or production firewall. Do not infer that UPnP mapping is verified from this NAT-PMP result; keep UPnP as a separate capability boundary.

For Wake-on-LAN behavior coverage, use `tools/lucky_wol_ci_probe.py` rather than a real LAN host. The probe is GitHub-Actions-only and starts pinned Lucky 3.0.0 on a uniquely named Docker `--internal` bridge with **no published admin port**. Through Lucky's API it verifies the fresh WOL Server/Client are disabled, temporarily enables only the Server, creates one owned device with a locally administered random MAC and the internal bridge broadcast address, and requires `CanWakeup=true` before calling `GET /api/wol/device/wakeup?key=<owned key>`. A raw AF_PACKET capture is bound only to that TEST bridge and must observe the exact standard 102-byte magic payload. Current Lucky 3.0.0 emits that wake datagram on **UDP/9** even when the device `Port` field is set to another random value. Treat this as packet-emission verification only: the synthetic device reports `State=Unknown`, so do not claim a powered endpoint went online. Never call the shutdown route for coverage. Delete the TEST device, restore Server/Client to their exact disabled baseline, and remove the disposable bridge/container.
...
Treat Rclone SystemMount as deployment-capability-sensitive. On the current containerized Lucky deployment, a bounded TEST attempt with empty remote/sync baselines, owned global cache/upload paths, an owned local remote and an owned mount point was accepted but failed with `unmountConflictFail: operation not permitted`. Read-only container inspection showed `Privileged=false`, no `SYS_ADMIN` and no `/dev/fuse` device mapping. Do not add those production-container privileges merely for coverage. Record this as a blocked boundary and continue mount/unmount only on a dedicated FUSE-capable test instance; always restore global config and remove the TEST remote/path.
...
## Third-party OAuth / OIDC boundary
...
On the current Lucky 3.0.0 instance, an owner-authorized OIDC preflight proved that `OIDCAuthorizationEndpoint`, `OIDCClientID` and `OIDCRedirectURI` can be temporarily PUT, read back exactly and restored under an ownership guard while the third-party user-list baseline remains unchanged. However, calling `GET /api/oauth/tmpcode?type=oidc` through the normal OpenToken client still returned `ret=2` with no `tmpCode`, `authUrl` or `authServer`, even with valid isolated TEST public metadata. Treat actual OAuth authorization as **blocked in an OpenToken-only context** until a legitimate browser/admin-session-capable isolated environment is available. Do not use Playwright/Chromium, read Lucky internals, forge browser sessions or weaken auth controls merely to complete this coverage item.
