...
`GET /api/reboot_program` is also behavior-verified only on that disposable instance. Docker `restart=always` is used solely as the CI supervisor: the Lucky process PID changed, the login challenge recovered, and the changed password still logged in after restart. Never persist passwords, RSA login plaintext, TOTP secrets/codes or admin tokens in logs/evidence. Remaining self-update/certificate-destruction flows must follow the same disposable-CI rule.
