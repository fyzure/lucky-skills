...
当前目标为 **Lucky 3.0.0 wanji / Linux x86_64**。默认目录已将已知 `UNKNOWN` 路由完成归类，并对主要写接口和常用模块补充请求/响应 schema。静态前端快照原有 599 条 path+method 记录；经 GitHub-hosted runtime 校准后，2 条 Docker 前端调用被 private DinD + owned container 实测为稳定 HTTP 404，并以 `runtime-rejected` false positive 保留证据但从目标版本 merged catalog 抑制。最终 **597 条 merged route 全部为 `runtime-verified`、0 条 `frontend-call`、0 条 unknown**；共有 **243 条 POST/PUT/PATCH**，其中 **219 条**生成 OpenAPI `requestBody`，仅 **1 条**仍含未类型化顶层请求属性，显式 response schema 已覆盖 **354 条**路由。`lucky-route-method-ci` 在 pinned Lucky 3.0.0 的 Docker `--internal` 网络中、无登录且不启用 OpenToken，对方法存在性做 404/鉴权门校准；危险路由也只允许停在鉴权层，不执行 protected handler。`runtime-verified` 只表示目标版本的路由/方法或受控运行时行为已有证据，具体行为深度仍以逐路由 evidence 为准。
