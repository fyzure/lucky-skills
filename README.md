...
当前目标为 **Lucky 3.0.0 wanji / Linux x86_64**。默认目录已将已知 `UNKNOWN` 路由完成归类，并对主要写接口和常用模块补充请求/响应 schema。当前 599 条 merged route 中 **573 条为 `runtime-verified`、26 条仍为 `frontend-call`、0 条 unknown**；共有 **243 条 POST/PUT/PATCH**，其中 **219 条**生成 OpenAPI `requestBody`，仅 **1 条**仍含未类型化顶层请求属性，显式 response schema 已覆盖 **354 条**路由。新增的 GitHub-hosted `lucky-route-method-ci` 在 pinned Lucky 3.0.0 的 Docker `--internal` 网络中、无登录且不启用 OpenToken，对 76 个非 dangerous 候选做了 method/path 校准验证，其中 74 条获得运行时路由证据；`runtime-verified` 只表示目标版本的路由/方法或受控运行时行为已有证据，具体行为深度仍以逐路由 evidence 为准。
