...
`tools/lucky_wol_ci_probe.py` 已在 GitHub-hosted 的 disposable Lucky 3.0.0 上验证真实 wake 数据面与虚拟 powered-state：probe 创建唯一 Docker `--internal` bridge，Lucky admin 端口不 publish，临时 WOL device 使用 locally-administered 随机 MAC、该 bridge 的私网 broadcast address 和预留 TEST ProbeTarget；WOL Server 只在这个临时实例中由 API 从 disabled 切到 enabled。`GET /api/wol/device/wakeup?key=<TEST>` 返回 `ret=0` 后，raw capture **只绑定这张 internal bridge**，实际捕获到标准 102-byte magic packet（`ff`×6 + TEST MAC×16）。当前 Lucky 3.0.0 实测把 wake datagram 发往 **UDP/9**，即使 device 的 `Port` 字段设成另一个随机值也不会使用该值。结束后 device 被删除，Server/Client 恢复原 disabled 基线，虚拟 target、临时 Lucky 与 network 全部销毁。`shutdown` 的 `METHOD + path` 已由无鉴权 cloud CI 命中 Lucky 鉴权门，因此路由存在性已验证；真正的关机 handler 仍明确不执行。
...
`tools/lucky_update_ci_probe.py` 也只允许 GitHub Actions，并只在 fresh pinned Lucky 上验证自更新。官方 `lucky_2.27.2_Linux_x86_64.tar.gz` 可被 `/api/update` 正确解析为 Linux/amd64/2.27.2 元数据，staging 返回 `ret=0`；将该解析结果原样提交到 `PUT /api/update/comfire` 也得到 `ret=0`。但真实 3.0.0 → 2.27.2 downgrade 随后让 HTTP 服务中断，45 秒恢复窗口内 login/API 都没有回来；Docker supervisor 同时仍报告容器 running、RestartCount=0、ExitCode=0。该结果被固化为 **non-serving downgrade failure semantic**，不能解释为成功更新，更不能用旧版本包在生产环境做覆盖测试。

SSL destructive coverage 使用独立的 `tools/lucky_ssl_destructive_ci_probe.py`。probe 在 runner 内生成短命 self-signed 证书和私钥，fresh Lucky 的直接证书导入实际接受 `AddFrom=file`。对该 owned TEST item 调 `PUT /api/ssl/flush?key=...` 时，3.0.0 返回 `ret=1 / UnsupportedRefreshType file`；`ACMEing` 保持 false、SHA-256 指纹不变，因此这不是 renew 成功，而是 file 证书不支持 refresh 的明确业务语义。随后 `DELETE /api/ssl?key=...` 真实删除 item 并把 fresh SSL list 恢复为空。证书/私钥、DNS、ACME account、Cloudflare credential 和生产证书均不进入这个 probe。

`tools/lucky_docker_prune_ci_probe.py` 则把真实 prune 完全隔离到 GitHub Actions 的 private Docker-in-Docker daemon。Lucky 只挂载一个专门 named volume 中的 DinD Unix socket，runner/生产 `/var/run/docker.sock` 不会出现在 Lucky 容器里。`POST /api/docker/prune` 使用 `{all:true,volumes:true}` 后，owned stopped container、unused network、anonymous volume 和 dangling image 都真实消失，而 running protected container 及其 network/volume/image 保持存在。一个额外 tagged-but-unused image 仍被保留，BuildKit cache 数量也不变，所以 Lucky 3.0.0 的 `all=true` **不能按 `docker image prune -a` 理解**；这是经过真实 daemon 校正后的行为证据，不再沿用 mock 层的过度推断。

全局配置 restore 也已迁到独立的 `tools/lucky_config_restore_ci_probe.py`。fresh Lucky 先写入一个非敏感 profile A marker，再通过 authenticated `GET /api/configure` 导出 configuration archive；导出实测为 `application/octet-stream` attachment。随后把 marker 改为 profile B，使用 multipart `file` 字段 POST 回 `/api/configure`，成功响应给出 `file / restoreConfigureKey / ret` 三个顶层字段；probe 只保留字段名，不记录临时文件路径或 key 值。接着 `GET /api/restoreconfigureconfirm?key=<opaque>` 真正确认 restore，等待登录 challenge 恢复并重新登录后，marker 从 profile B 回到 profile A。整个 ZIP 只存在 runner 内存/临时目录并只回传给同一个 disposable Lucky，配置正文、秘密字段和 restore key 不进入日志/evidence。
...
StorageManagement 自身的 `SystemMount` 现在完成的是 **Linux target 平台边界**，不是实际 mount 成功。`tools/lucky_storage_mount_ci_probe.py` 从 fresh pinned Lucky 3.0.0 的 served frontend 提取到该表单只在 `Type != "local" && os == "windows"` 时渲染，并直接提示 WinFsp；保存前的 `Ze()` validator 只检查 Remark、local path、WebDAV 与阿里云盘字段，并不包含 SystemMount 校验。为了排除“只是 Linux 容器没 FUSE 权限”的可能，CI 仍给 disposable 容器 `SYS_ADMIN`、`/dev/fuse` 与 unconfined AppArmor，再强制 POST 一个仅属于 probe 的 local SystemMount。后端在创建 storage item 前稳定返回 `ret=4` / `mountpoint format error`，列表中没有 TEST item，cleanup 后 storage/litelist/Cron/path 基线全部恢复。因此当前 Linux x86_64 目标不把这个 Windows/WinFsp 路径当作未完成覆盖；若未来要宣称 StorageManagement 实际 SystemMount，则必须另建 Windows Lucky + WinFsp + 非 local storage 的受控测试环境。Rclone 的 SystemMount 是另一套 Linux 可用能力，见下文独立 FUSE probe。
...
Rclone SystemMount 现在也完成了独立行为闭环，但只在 **GitHub-hosted disposable Lucky 3.0.0 + FUSE 专用容器**中执行，不给生产 Lucky 提权。`tools/lucky_rclone_mount_ci_probe.py` 给一次性容器增加 `SYS_ADMIN`、`/dev/fuse` 与 unconfined AppArmor，然后仍只通过 Lucky HTTP API 创建 owned `/tmp/TEST-*` source/mount/cache/upload 路径、Cron marker 和 local remote。一个关键运行时语义是：local remote 的真实挂载源路径位于 `SystemMount.Root`；顶层 remote `Root` 或 StorageManagement 风格的 `Params.LocalPath` 都不会替代该字段。正确设置后，source marker 能从 mount point 看到；通过 mount point 写入第二个 marker 后，source 侧也立即可见。随后 remote disable 会让 mount 消失，DELETE remote、恢复 global config、删除 Cron helper/path 后，remote/sync/Cron/group/path 基线全部恢复。RS/生产 Lucky 仍维持无 `SYS_ADMIN`、无 `/dev/fuse` 的原容器能力。
...
全新 TEST DB 可直接用 FileBrowser 文档化的 reset-default `666/666` 登录；行为 probe **不会执行** Lucky 的高风险 `resetadmin` handler。该 `GET /api/third/filebrowser/resetadmin` 的 METHOD+path 存在性已经由无鉴权 GitHub-hosted route-method CI 命中 Lucky 鉴权门，因此这里的限制只针对破坏性业务行为，不代表路由未验证。登录得到的 JWT 只保存在进程内并作为 `X-Auth` 使用。实际文件闭环已经验证：raw POST upload 返回 200、资源 GET 返回 200 并读回相同 TEST 内容、PATCH rename 返回 200，DELETE 正常成功码为 **204**，对应 Lucky-visible backing file 同步创建、改名并消失。恢复前 probe 会重新确认 DB path、listener port 和 mount Param 仍是本次 TEST ownership marker，只有匹配时才 PUT 回原配置；最终 stopped 状态、完整配置和 TEST path 均恢复基线。
