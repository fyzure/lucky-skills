...
| WOL | 已验证 GitHub-hosted 临时 Lucky 3.0.0 + Docker internal bridge 上的真实 wake packet emission 与虚拟 powered target 状态变化：wake 前 `Unreachable`，精确 102-byte magic packet / UDP/9 后夹具启动固定 TEST IP/MAC，Lucky 回读 `Reachable` 且 `ReachableTargetList` 命中；`shutdown` 明确排除在覆盖目标之外 |
...
- 599 条 merged route 的证据等级为 **573 条 `runtime-verified` + 26 条 `frontend-call` + 0 条 `unknown`**；其中 GitHub-hosted `lucky-route-method-ci` 在 fresh pinned Lucky 3.0.0 的 Docker `--internal` 网络中、未登录且未启用 OpenToken，对剩余非 dangerous 候选做 404/鉴权控制校准，新增 74 条 `METHOD + path` 运行时存在性证据。runtime sidecar 中的 `routes[]` 与 `route_method_ci_evidence` 都会覆盖静态 `frontend-call` 证据等级，但是否真正执行了成功业务行为仍以逐路由 `verification` / `schema_evidence` 为准。
